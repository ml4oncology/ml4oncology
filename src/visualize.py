...
from src.utility import (load_ml_model, get_clean_variable_names)

###############################################################################
# Feature Importance
###############################################################################
def importance_plot(algorithm, target_events, save_dir, figsize, top=20, 
                    importance_by='feature', padding=None, colors=None):
    ...
    for idx, target_event in tqdm.tqdm(enumerate(target_events)):
        ...
        summary[target_event] = [f'{feature} ({importance})' for feature, importance in 
        ...
        fig.savefig(f'{save_dir}/figures/important_{importance_by}s/{algorithm}_{target_event}.jpg', 
...
###############################################################################
def subgroup_performance_plot(df, target_event='ACU', subgroups=None, padding=None, 
                              figsize=(16,24), save=False, save_dir=None):
    if save_dir is None: 
        if save: raise ValueError('Please provide save_dir if you want to save figures')
    ...
    for i, metric in enumerate(metrics):
        ...
        if save: fig.savefig(f'{save_dir}/{target_event}_{metric}.jpg', 
                             bbox_inches=get_bbox(axes[i], fig, **padding), dpi=300) 
    if save: 
        plt.savefig(f'{save_dir}/{target_event}.jpg', bbox_inches='tight', dpi=300)
    plt.show()
...
###############################################################################
def service_request_plot(summary):
    fig, axes = plt.subplots(nrows=1, ncols=2, figsize=(15,6))
    ylabel = lambda col: f'{col} Prior to Treatment\nCount (NSessions)'
    for i, col in enumerate(['Requested Service', 'Did Not Request Service']):
        summary.T[col].unstack().plot(kind='bar', ylabel=ylabel(col), ax=axes[i])
    plt.show()
...
def tree_plot(train, target_event='Neutropenia', algorithm='RF'):
    ...
    if algorithm == 'RF':
        tree.plot_tree(clf, feature_names=feature_names, 
                       class_names=[f'Not {target_event}', target_event], fontsize=10, filled=True, ax=ax)
    else:
        plot_tree(clf, ax=ax)
