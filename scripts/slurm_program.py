...
class End2EndPipeline():
    ...
    def get_train(self, rnn=False, processes=64):
        if rnn:
            ...
            dataset = (drop_ikn(X_train), drop_ikn(X_valid), drop_ikn(X_test), Y_train, Y_valid, Y_test)
    ...
    def evaluate(self):
        ...
        eval_models = Evaluate(output_path=self.output_path, preds=ens.preds, labels=ens.labels, orig_data=None)
